from utilslegend.CheckSideUtils     import CheckSideLine
    ...
    def update(self, trackerlist) :
        for object_track in trackerlist :
            ...
            if (object_track.objectID not in self.listObject) :
                ...
                # if(self.isinside(object_track.bbox)) and object_track.appeared >= 3:
                    self.counter +=1
...
class Counter_LineOneWay() :
    ...
    def update(self, trackerlist) :
        for object_track in trackerlist :
            ...
            # if (object_track.objectID not in self.listLR) and len(object_track.trace) > 2 :
            if len(object_track.trace) > 2 and self.isinside(object_track.trace[-1]):
                if self.line.check_right(object_track.trace[-2], object_track.trace[-1]) and self.line.not_same_side(object_track.trace[0], object_track.trace[-1]):
                    self.counterLR +=1
...
class Counter_Line2Way() :
    ...
    def update(self, trackerlist) :
        ...
        for object_track in trackerlist :
            ...
            # if (object_track.objectID not in self.listRL) and len(object_track.trace) > 2 :
